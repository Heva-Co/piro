...
/// (from the host) pointed at the 3LO gateway for the integration's cloudId.
/// </summary>
internal static class JiraOptionsHttp
{
    public static async Task<(HttpClient Http, string CloudId)> CreateAsync(
        IIntegrationHost host, Guid integrationId, CancellationToken ct)
    {
...
public sealed class JiraProjectsOptionsProvider : IOptionsProvider
{
    ...
    {
        var (http, cloudId) = await JiraOptionsHttp.CreateAsync(host, integrationId, ct);
...
/// </summary>
public sealed class JiraIssueTypesOptionsProvider : IOptionsProvider
{
    public string IntegrationId => "Jira";
