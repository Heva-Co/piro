<script lang="ts">
  ...
  import type { TriggerDto } from "$lib/api.js";
  ...
  const filtered = $derived(
    statusFilter === "all"
      ? triggers
      : triggers.filter((t) => (statusFilter === "active" ? t.status !== "INACTIVE" : t.status === "INACTIVE"))
  );
  ...
  async function load() {
    ...
    try {
      ...
      else triggers = result;
    } catch { error = "Failed to load triggers."; }
...
<div class="flex w-full flex-col gap-6 p-4">
  <div class="flex items-center justify-between">
    ...
    <Button onclick={() => goto("/admin/triggers/new")}>
  ...
  {:else if triggers.length === 0}
    <div class="rounded-lg border border-dashed flex flex-col items-center justify-center py-16 gap-3 text-center">
      ...
      <Button size="sm" onclick={() => goto("/admin/triggers/new")}>
        <PlusIcon class="size-4 mr-1" /> Create your first trigger
      </Button>
    </div>
  {:else}
        ...
        <Table.Body>
          {#each paged as t (t.id)}
            <Table.Row>
              ...
              <Table.Cell>
                <div class="flex items-center gap-1.5">
                  ...
                  {#if t.status !== "INACTIVE" && t.alertConfigCount === 0}
                    <span class="inline-flex items-center gap-1 text-xs text-amber-500" title="No alert configs linked — this trigger won't fire">
              ...
              <Table.Cell>
                <Button variant="ghost" size="sm" onclick={() => goto(`/admin/triggers/${t.id}`)}>
