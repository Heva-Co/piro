...
- **A `.tsx` with a single component uses `export default`, declared at the end of the file.** When a file exports exactly one component (the common case, given the one-component-per-file rule above), declare the component as a plain function and put the `export default` on its own line **after** the component (and any local helpers): `function Foo(props: Props) { ... }` … then `export default Foo;` at the bottom. Do **not** inline it in the signature (`export default function Foo`). Named exports are for files that legitimately export several things (e.g. a barrel, or a hook + its types). Consistent default exports keep imports uniform (`import Foo from ".../Foo"`).
