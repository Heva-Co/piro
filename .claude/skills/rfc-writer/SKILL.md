...
- **Design**: number a subsection per real component touched, each naming the actual interface/entity/dispatcher involved and how the new work plugs into it. Include a flow diagram — reviewers read the diagram before the prose. **Use a Mermaid diagram in a ` ```mermaid ` fenced block, not ASCII art** — GitHub renders Mermaid natively, so it's legible in the PR/blob view where these RFCs are actually read. Pick the diagram type that fits: `sequenceDiagram` for a request/response or multi-actor flow (SPA → controller → service → email, etc.), `flowchart TD/LR` for branching logic or pipeline routing, `erDiagram` for new entity relationships, `stateDiagram-v2` for a status/lifecycle machine. Keep node labels short; put detail in the prose, not the diagram. (Older RFCs in `docs/rfcs/` still use ASCII boxes — do not copy that; Mermaid is the current convention.) End with an explicit **"What does NOT change"** subsection — this is not filler, it's how a reviewer judges blast radius. List the interfaces/pipelines you are deliberately leaving untouched and why reusing them (instead of adding a parallel path) is the point.
